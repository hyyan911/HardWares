...
{
    internal class PortResourceTCPHelper : PortResourceHelper
    {
        public PortResourceTCPHelper(Func<string, string> getProductName) : base(getProductName)
        {
        }

        #region COM部分

        public string GetIdentifier(TCPIPInternalInterface device, List<object> param)
        {
        ...
        /// 获取USB设备
        /// </summary>
        public List<string> EnumerateDevices(TCPIPInternalInterface device)
        {
            List<string> res = device.GetRawTCPIPDeviceNames();
